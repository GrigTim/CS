//Задача 54 упорядочит по убыванию элементы каждой строки двумерного массива.
...
Console.WriteLine(@"Задача 54 упорядочит по убыванию элементы каждой строки двумерного массива.");
...
// вот туть и происходит сортировка 
void RepElRows(int[,] inArray)
{    
    for (int i = 0; i < inArray.GetLength(0); i++)
    {
        for (int j = 0; j < inArray.GetLength(1); j++)
        {
            int max = j;
            for (int count = j+1; count< inArray.GetLength(1); count++)
            {
                if(inArray[i,count] > inArray[i,max])
                max = count;
            } 
